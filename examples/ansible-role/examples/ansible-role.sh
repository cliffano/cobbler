...
mkdir -p ../stage/test-examples/roles/cliffano.cobblerexample
for dir_name in config defaults meta tasks vars; do
	mkdir -p ../stage/test-examples/roles/cliffano.cobblerexample/"${dir_name}"
	cp -R "${PWD}"/../"${dir_name}"/ ../stage/test-examples/roles/cliffano.cobblerexample/"${dir_name}"
done
